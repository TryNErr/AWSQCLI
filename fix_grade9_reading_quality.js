...
/**
 * GRADE 9 READING QUALITY FIX
 * 
 * Replaces inappropriate and duplicate questions with proper Grade 9 level content
 */

const fs = require('fs');
const path = require('path');

// Grade 9 appropriate reading passages and questions
const grade9ReadingQuestions = [
  {
    "_id": "grade9_medium_reading_001",
...
// Additional questions to reach a good variety
const additionalQuestions = [
  {
    "_id": "grade9_medium_reading_011",
...
const allQuestions = [...grade9ReadingQuestions, ...additionalQuestions];

function fixGrade9ReadingQuestions() {
  const filePath = '/workspaces/AWSQCLI/testace-app/public/questions/9_medium_reading.json';
  
  try {
    console.log('🔧 Fixing Grade 9 Medium Reading Questions...');
    
    // Write the new appropriate questions
    fs.writeFileSync(filePath, JSON.stringify(allQuestions, null, 2));
    
    console.log('✅ Successfully replaced inappropriate questions with Grade 9 level content');
    console.log(`📚 Added ${allQuestions.length} appropriate reading comprehension questions`);
    console.log('🎯 Questions now include:');
    console.log('   - Complex passages with multiple paragraphs');
    console.log('   - Literary device identification');
    console.log('   - Critical thinking and inference');
    console.log('   - Text structure analysis');
    console.log('   - Author\'s purpose and tone');
    console.log('   - Scientific and historical content');
    console.log('   - Current events and cultural topics');
    
    // Verify the fix
    const verifyContent = fs.readFileSync(filePath, 'utf8');
    const questions = JSON.parse(verifyContent);
    
    // Check for any remaining inappropriate content
    const inappropriateQuestions = questions.filter(q => 
      ...
      q.content.length < 100 // Very short questions are likely too simple
    );
    
    if (inappropriateQuestions.length === 0) {
      console.log('✅ Verification passed: No inappropriate content found');
    } else {
      console.log(`⚠️  Warning: ${inappropriateQuestions.length} potentially inappropriate questions still exist`);
    }
    
    console.log('\n🎓 Grade 9 Reading Questions Quality Summary:');
    console.log(`   - Total questions: ${questions.length}`);
    console.log(`   - Average content length: ${Math.round(questions.reduce((sum, q) => sum + q.content.length, 0) / questions.length)} characters`);
    console.log(`   - Topics covered: ${new Set(questions.map(q => q.topic)).size} different topics`);
    
  } catch (error) {
    console.error('❌ Error fixing Grade 9 reading questions:', error.message);
    process.exit(1);
  }
}

// Run the fix
fixGrade9ReadingQuestions();
